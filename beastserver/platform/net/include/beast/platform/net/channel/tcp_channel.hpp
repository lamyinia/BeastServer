...
/**
 * TcpChannel：pipeline 上层；IO 串行化由 Session strand（Transport 借用）保证。
 */
class TcpChannel final : public IChannel {
public:
    explicit TcpChannel(std::shared_ptr<transport::TcpTransport> transport);
    ...
    void rebind_transport(std::shared_ptr<transport::TcpTransport> transport);
    [[nodiscard]] std::shared_ptr<transport::TcpTransport> transport() const { return transport_; }
...
private:
    ...
    std::shared_ptr<transport::TcpTransport> transport_;
