...
void WebsocketTlsTransport::start(OnBytes on_bytes, OnClosed on_closed, OnError on_error) {
    boost::asio::post(strand_, [self = shared_from_this(), this,
        ...
        if (started_) {
            BEAST_LOG_WARN("WebsocketTlsTransport start() called twice");
            return;
        }
        started_ = true;
...
void WebsocketTlsTransport::send(Bytes&& data) {
...
void WebsocketTlsTransport::close() {
    boost::asio::post(strand_, [self = shared_from_this(), this]() { do_close(); });
}

bool WebsocketTlsTransport::is_closed() const noexcept {
    return closed_;
}

WebsocketTlsTransport::Strand WebsocketTlsTransport::strand() const {
    return strand_;
}

void WebsocketTlsTransport::do_read() {
    if (closed_) {
        return;
    }
...
void WebsocketTlsTransport::on_read_complete(boost::beast::error_code ec, std::size_t bytes_transferred) {
    if (closed_) {
        return;
    }
    if (ec) {
...
void WebsocketTlsTransport::do_write() {
    if (closed_) {
        writing_ = false;
        return;
    }
...
void WebsocketTlsTransport::on_write_complete(boost::beast::error_code ec) {
    if (closed_) {
        writing_ = false;
        return;
    }
...
void WebsocketTlsTransport::do_close() {
    if (closed_) {
        return;
    }
    closed_ = true;

    // 优雅关闭：先发 WebSocket close frame（best-effort），再 shutdown TLS，最后关 socket
    boost::beast::error_code ignored;
    stream_.close(boost::beast::websocket::close_code::normal, ignored);

    // SSL 层 shutdown（best-effort，对端可能已断开）
    if (stream_.next_layer().lowest_layer().is_open()) {
        stream_.next_layer().shutdown(ignored);
        stream_.next_layer().lowest_layer().cancel(ignored);
        stream_.next_layer().lowest_layer().close(ignored);
    }
