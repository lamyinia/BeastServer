...
void check_runtime_env() {
    // 运行时环境检查：1000 bot 场景需要 ulimit -n >= 65535
    // 这里只 WARN，不强制阻止启动
    struct rlimit rl{};
    if (getrlimit(RLIMIT_NOFILE, &rl) == 0) {
        if (rl.rlim_cur < 65535) {
            BEAST_LOG_WARN(
                "stress_event: RLIMIT_NOFILE={} < 65535, 1000 bot 压测可能 EMFILE",
...
void StressEventEngine::on_start(beast::platform::engine::context::EngineContext& ctx) {
    ctx_ = &ctx;
    check_runtime_env();
    BEAST_LOG_INFO(
        "stress_event engine started instance={} (静默日志模式，echo/compute 不打 INFO)",
        ctx.instance_id());
}

void StressEventEngine::on_event(const beast::platform::engine::instance::InstanceEvent& event) {
...
void StressEventEngine::on_echo(
...
void StressEventEngine::on_compute(
    const beast::platform::PlayerId& player_id,
...
void StressEventEngine::on_metrics_query(
    const beast::platform::PlayerId& player_id,
...
std::unique_ptr<StressEventEngine> make_stress_event_engine() {
    return std::make_unique<StressEventEngine>();
}
